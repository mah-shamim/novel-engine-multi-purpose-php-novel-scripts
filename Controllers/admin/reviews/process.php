...
$orderDirection = $_POST['order'][0]['dir'];
    ...
    $db->table("$table as r")->leftJoin('users as u', 'r.user_id', '=', 'u.id')->leftJoin('ebook as e', 'r.file_id', '=', 'e.id')->select(['r.status', 'r.content', 'r.id', 'r.created_at', 'r.file_id', 'u.name as user_name', 'e.slug as e_slug', 'e.name as e_name']);
    ...
    if (!empty($searchValue)) {
        $db->search(['u.name', 'e.name'], $searchValue);
    }

    $columns = array('r.id', 'u.name', 'e.name', 'r.status', 'r.created'); 
    ...
    $dropDown .= '<a data-id="' . $row['id'] . '" class="dropdown-item d-flex align-items-center waves-effect text-danger delete " href="#">Delete</a>
        ...
        $link = '<a href="javascript:void(0);" class="show-review" data-file_id="'.$row['file_id'].'" data-id="'.$row['id'].'" data-content="'.htmlspecialchars($row['content']).'">'.$row['e_name'].' </a>';
