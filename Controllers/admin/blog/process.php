...
$Star = new App\General\Admin\Blogs();
...
$orderDirection = $_POST['order'][0]['dir'];
    ...
    $db->table("$table as p")->leftJoin('cats as c', 'p.cid', '=', 'c.id')
    ...
    if (!empty($searchValue)) {
        $db->search(['p.name'], $searchValue);
    }

    $columns = array('p.id', 'p.name',  'p.status', 'p.created'); 
    ...
    if(INSTANT_INDEXING === 1) {
         $dropDown .= '<a data-id="' . $row['id'] . '" class="dropdown-item d-flex align-items-center waves-effect text-info index " href="#">Index now</a>
    </li>';
    }

    $dropDown .= '<li><a data-id="' . $row['id'] . '" class="dropdown-item d-flex align-items-center waves-effect text-danger delete " href="#">Delete</a>
    </li>
    ...
    $gen = new App\General\All();
    $results = $gen->listrow('cats', 'id');
    
    $data = [];
    
    if(!empty(Input('id'))) {
        $cid = $gen->getRowData($table, 'cid', 'id', Input('id'));
        $cat = $gen->getRow('cats', 'id', $cid);
        if ($results !== false) {
