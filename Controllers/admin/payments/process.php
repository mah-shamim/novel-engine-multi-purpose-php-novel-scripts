...
$orderDirection = $_POST['order'][0]['dir'];
    ...
    $db->table("$table as p")
                ...
                ->leftJoin('packages as pa', 'p.package_id', '=', 'pa.id')
                ->select(['p.*', 'pa.package_name', 'u.username']);
    ...
    if (!empty($searchValue)) {
        $db->search(['pa.package_name', 'u.username'], $searchValue);
    }

    $columns = array('p.id', 'pa.package_name', 'u.username', 'p.reference', 'p.status', 'p.created'); 
