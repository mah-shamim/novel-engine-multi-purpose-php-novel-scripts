...
$sql = "SELECT s.*, p.package_name, p.price, u.username 
    ...
    if (!empty($searchValue)) {
        $sql .= "AND (p.package_name LIKE :searchValue OR u.username LIKE :searchValue) ";
    }



    $columns = array('s.id', 'image', 'name', 'status', 'isHome'); 
    ...
    foreach ($results as $row) {
        ...
        if (empty($row['image'])) {
            ...
            $thumb = '<img class="border border-dark" src="'.APP_URL.'/Public/thumb/users/'.$row['image'].'" alt="Icon" width="60" height="60">';
        }
        $status = ($row['status'] == 1 ? '<span class="badge bg-success">Active</span>' : ($row['status'] == 2 ? '<span class="badge bg-danger">Expired</span>'  : '<span class="badge bg-warning">Trashed</span>'));
    ...
    if ($row['status'] != 1) {
        ...
        $dropDown .= '
        <li><a class="dropdown-item d-flex align-items-center waves-effect text-warning trash" href="#" data-id="'.$id.'">Deactivate</a></li>
        <li>';
    }
    ...
    $dropDown .= '<a data-id="' . $row['id'] . '" class="dropdown-item d-flex align-items-center waves-effect text-danger delete " href="#">Delete</a>
        ...
        $rowData = array(
            ...
            '5' => $status,
            '6' => $dropDown,
            '7' => date('D m, Y', strtotime($row['created_at']))
        );
