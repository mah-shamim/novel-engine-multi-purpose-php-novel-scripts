...
$orderDirection = $_POST['order'][0]['dir'];
    ...
    $db->table("$table as r")
                ->leftJoin('blogs as b', 'r.post_id', '=', 'b.id')
                ->select(['r.*', 'b.name as post_name']);
    ...
    if (!empty($searchValue)) {
        $db->search(['r.name', 'r.email'], $searchValue);
    }

    $columns = array('r.id', 'r.name', 'r.email', 'r.status', 'r.created'); 
    ...
    $dropDown .= '<a data-id="' . $row['id'] . '" class="dropdown-item d-flex align-items-center waves-effect text-danger delete " href="#">Delete</a>
        ...
        $link = '<a href="javascript:void(0);" class="show-review" data-file_id="'.$row['post_id'].'" data-id="'.$row['id'].'" data-content="'.htmlspecialchars($row['content']).'">'.$row['post_name'].' </a>';
